...
from models import db, Wallet, Beneficiary, Transaction
...
def beneficiaries(current_user):
    if request.method=='GET':
        ...
        return jsonify([{'id':x.id,'name':x.name,'ref':x.account_reference} for x in b])
    d = request.json
    new = Beneficiary(
      name=d['name'],
...
def txns(current_user):
    ...
    return jsonify([
      {'to':x.to_beneficiary,'amt':x.amount,'fee':x.fee,'when':x.timestamp}
      for x in t
    ])
